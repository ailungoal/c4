...
public:
    IntImage():variance(0.0),label(-1) { }//初始化，初始化列表在构造函数执行前执行
    virtual ~IntImage() { Clear(); }

    virtual void Clear(void);//虚函数实现多态，父类必须写virtual
    ...
    void CalcIntegralImageInPlace(void);//void参数表示此处不能传入参数，空的可以传任何参数
...
public:
    ...
    using Array2dC<T>::p; //在子类中使用using申明引入基类成员名称
    REAL variance;
    int label;
};
