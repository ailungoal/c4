...
{
	image = original;
	calculate_sobel_gpu(image,ct,image.nrow,image.ncol);
	//calculate_sobel(image,sobel);
	//calculate_ct(sobel,ct);
...
void DetectionScanner::InitIntegralImages(const int stepsize)
{
	...
	for(int i=0;i<xdiv-EXT;i++)
	{
		...
		for(int j=0;j<ydiv-EXT;j++)
		{
			...
			linearweights += baseflength;//svm权重被分割为24个
		}
	}
	scores.CalcIntegralImageInPlace();  //计算分数的积分图
	//calculate_integral(scores,wd,hd);
...
int DetectHuman(const char * filename,const char * outname,DetectionScanner& ds,std::ofstream& out)
{
    ...
    if(outname)
    {
		...
		for(unsigned int i=0;i<results.size();i++)
			rectangle(img,Point(results[i].left,results[i].top),Point(results[i].right,results[i].bottom),Scalar(255,0.0),2);
		imshow(filename,img);
		waitKey(1);		
    }
...
void RunFiles()
{
	...
	/*std::ofstream out("result\\result_HIK.txt");
	cv::VideoCapture capture("C:\\Users\\Administrator\\Desktop\\20171016_2.avi");
	if(capture.isOpened())
	{
		...
		for(int i=0;i<totalFrame;i++)
		{
			cv::Mat src;
			capture.read(src);
			DetectHuman(src,scanner,out);
		}
	}*/
    const std::string base_dir = "";
    const char* out_dir = "";
    const char* listname = "result\\file.txt";
    ...
    while(in.good())
    {
		...
		cost=end-start;
		std::cout<<"Processed "<<numfile<<" images in "<<cost<<" milliseconds."<<std::endl;
        numfile++;
    }
